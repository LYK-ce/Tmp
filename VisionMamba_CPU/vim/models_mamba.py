...
def create_block(
    ...
    use_simd_scan=False,  # 新增：使用SIMD优化的selective scan
):
    if if_bimamba:
        bimamba_type = "v1"
    if ssm_cfg is None:
    ...
    # import ipdb; ipdb.set_trace()
    mixer_cls = partial(Mamba, d_state=d_state, layer_idx=layer_idx, bimamba_type=bimamba_type, if_divide_out=if_divide_out, init_layer_scale=init_layer_scale, use_cpp_scan=use_cpp_scan, use_fixlen_scan=use_fixlen_scan, use_fused_bidirectional=use_fused_bidirectional, use_full_cpp=use_full_cpp, use_simd_scan=use_simd_scan, **ssm_cfg, **factory_kwargs)
